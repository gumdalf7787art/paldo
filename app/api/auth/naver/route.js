...
export async function GET(request) {
  ...
  if (error) {
    return Response.redirect(`${origin}/login?error=${encodeURIComponent(error_description || error)}`, 302);
  }
  ...
  try {
    ...
    const tokenResponse = await fetch('https://nid.naver.com/oauth2.0/token', {
      ...
      body: new URLSearchParams({
        ...
        client_id: 'Q2vmXowiBqzcUgm13IF0',
        client_secret: 'k_kX6ewBwC',
        code: code,
        state: state || ''
      })
    });
    ...
    if (!userResponse.ok || userData.resultcode !== '00') {
      return Response.redirect(`${origin}/login?error=${encodeURIComponent(userData.message || '사용자 정보 획득 실패')}`, 302);
    }
